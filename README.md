...
| LB / `Enter` / click | Toggle the highlighted action in/out of this preset |
...
LB, not A, is the documented way to toggle an action here - on one player's setup, A reliably worked on every other button in this project (Quick Menu, preset manager, Y/B on this very screen) but never once registered for toggling a row specifically, matching the same kind of single-button unreliability (likely a Steam Input mapping quirk) seen earlier with X for delete. Unlike X, though, A has no keyboard/tap fallback of its own for this action to lean on, so rather than keep chasing why A specifically doesn't reach this one handler, LB - unused elsewhere in this menu - took over as the real, documented button. A is still wired the same way in code (there's no reason to assume it's broken on every setup, just this one), just not advertised.

This is now the only place a preset can be deleted from. **Deletion is locked by default every time this screen opens**, and RT (or `L`) has to unlock it first - locked, LT just shows a message telling you to unlock it first. Once unlocked, LT deletes immediately with no separate confirmation step; LB/A keep their normal job (toggling an action) regardless of the lock state, so there's no risk of an accidental press deleting anything. This replaced an earlier design where several buttons (X/LB/RB/LT/right-click, and A too once unlocked) all triggered the same delete, layered under a request-then-confirm step on top of the lock: in testing, the lock toggle worked reliably, but no button reliably triggered the confirm step afterwards, and by that point the lock was already doing the job a confirm step is for (a deliberate, distinct action before anything destructive can happen) - so both the confirm step and the extra alternative buttons were dropped in favor of one dedicated button (LT) for one dedicated job.
...
`Framework/RadialMenu.cs`, wired up in `ModEntry.OnUpdateTicked`, is a first pass at "hold a button to pop up a radial menu, tilt the stick toward an entry, release to trigger it." It's disabled by nothing (there's no on/off switch yet) but effectively opt-in, since its hold-button defaults to `ControllerBack` - a button nothing else in this project uses - so it won't fire unless you rebind it. `RadialMenuButton` in `config.json` accepts any digital `SButton`, not just face/shoulder buttons - a d-pad direction (e.g. `"DPadRight"`) works fine too, which matters on setups where a Steam Input layout doesn't expose an obvious "Back" button to hold. Selecting an entry once it's open prefers the right stick, falling back to the left stick and then mouse position if the right stick isn't tilted (`ModEntry.GetRadialDirection`) - useful if a Steam Input config already dedicates the right stick to something else it can share, or frees it up entirely for this. Its entries come from a dedicated, always-present preset (`PresetManager.RadialPresetName`, "Radial Menu" - see Presets above), not whatever preset happens to be active in the Quick Menu - a dozen-mod "All" view would make for very cramped wedges, and reading from the active preset would mean the radial menu's contents changed every time you switched presets elsewhere, which isn't what you'd expect from something meant to be a small, curated, always-the-same set of go-to actions.
...
Follow-up on the A report above: everything else on that screen (Y, B, Enter/tap) was confirmed still working, narrowing it to A specifically - the same single-button-unreliability pattern already seen with X for delete, just without a keyboard/tap fallback of its own to lean on for this particular action. Rather than dig further into why A doesn't reach this one handler on this setup, gave it a dedicated alternative: LB (unused elsewhere in this menu) now does the same thing and is what the hint text and docs actually advertise; A is left wired too, just not documented, since there's no reason to think it's broken on every setup.

Also fixed a real rendering bug in `PresetNamePrompt`, found by decompiling `TextBox.Draw`: passing `null` for the box texture (which this prompt did, copying the same call vanilla `NamingMenu` makes) makes `TextBox` fall back to drawing `Game1.drawDialogueBox` at a hardcoded offset (`X - 32, Y - 112 + 10`) and size (`Width + 80`) relative to the textbox's own coordinates. That fallback only looks right in `NamingMenu` because it also sets a matching, much taller `Height` (192) built around exactly that offset - this prompt's much shorter box (48) meant the fallback frame rendered detached from where the actual text and click targets were, while typing itself kept working fine since text input goes through a completely separate path (`Game1.keyboardDispatcher.Subscriber`) that never touches `Draw` at all. Fixed by giving the `TextBox` the real small-box texture the game's own chat box uses (`LooseSprites\chatBox`) instead of `null`, which renders correctly at any height and avoids that fallback path entirely.

Per request, rebound the radial menu's hold-button on this Deck from `ControllerBack` to `DPadRight` (`config.json` only - `RadialMenuButton` was already fully config-driven, so this needed no code change) after `ControllerBack` turned out to have no obvious equivalent in this player's Steam Input layout. Selecting an entry with the right stick specifically was also requested but turned out to already be exactly what `ModEntry.GetRadialDirection` does (right stick first, left stick and then mouse as fallbacks) - no change needed there, just confirmation it was already built that way.

What a compile check and this playtesting still don't cover: whether LT reliably deletes across other presets and other sessions (confirmed for one so far), whether the XTest timing fix actually resolves triggering, whether LB reliably toggles actions where A didn't, whether the naming prompt now renders correctly, and whether the radial menu's direction math and the new `DPadRight` binding work well together in practice (D-pad right also being a normal movement input outside menus). All need another real test.
